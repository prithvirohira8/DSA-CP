...
// and not in the same direction. In this case it will not be considered to be a cycle.
// Hence for detecting a cycle in directed graphs we use a dfsVisited array along with 
// the normal visited array. 
// The dfsVisited and normal visited array are updated each time
...
// is explored entirely before moving ahead. 
// While backtracking from a node we set the dfsVisited as false again
...
public class Cycle_Detection_in_Directed_Graph_using_DFS {
  ...
  static boolean checkCycle(List<List<Integer>> adj, boolean[] vis, boolean[] dfsVis, int source) {
    ...
    for (int i = 0; i < adj.get(source).size(); i++) {
      ...
      if (!vis[child]) {
        if (checkCycle(adj, vis, dfsVis, child))
          return true;
