...
public class Solution {
    static boolean checkCycle(List<List<Integer>> adj, boolean[] visited, int source){
        ...
        while(!q.isEmpty()){
            ...
            for(int i = 0; i < adj.get(child).size(); i++){
                ...
                // the children of the parent are being iterated upon, the children of the parent,
                // will then consider the parent as its own child which is when the below condition
    ...
    public static String cycleDetection(int[][] edges, int n, int m) {
        ...
        for(int i = 1; i <= n; i++){
            ...
            if(checkCycle(adj, visited, i)) return "Yes";
        }
        return "No";
    }
}
